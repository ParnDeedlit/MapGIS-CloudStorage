  ...
  <div style="margin-left:25px;margin-right:25px;margin-top:15px;">
    <Row :gutter="16">
      <i-col span="16">
        ...
        <Modal title="文件磁盘分布" v-model="showFilePie" class-name="vertical-center-modal">
...
export default {
  ...
  watch: {
    $route() {
      ...
      // 复制到下一层的文件夹
      this.copyParams.dist =
        this.$route.params.id + "\\\\" + this.copyParams.name;
      setTimeout(() => {
  ...
  methods: {
    ...
    initFilePie(row) {
      ...
      this.chartData = {
        ...
        rows: [
          {
            使用量:
              this.toMem(this.currentDisk.Size - this.currentDisk.FreeSpace) +
              " 已使用",
            字节:
              this.currentDisk.Size -
              this.currentDisk.FreeSpace -
              this.currentDisk.FreeSpace / 50
          },
          {
    ...
    tableClick(row) {
      ...
      self.showFileDetail = true;
      console.log("tableClick", row, self);
    },
    createNewOne(path) {
      ...
      let pasteMenu = new MenuItem({
        ...
        click() {
          if (me.copyParams.type === "文件夹") {
            if (me.cut.path) {
              copyFolder(me.copyParams.src, me.copyParams.dist, dialog).then(
                result => {
            ...
            if (me.cut.path) {
              ...
              copyFile(me.copyParams.src, me.copyParams.dist, dialog).then(
                result => {
                  me.tableData.push(result);
                }
              );
